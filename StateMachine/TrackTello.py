...
blueLower= np.array([80,40,0], dtype = "uint8")
blueUpper = np.array([255,150,70], dtype = "uint8")
redLower = np.array([5,5,110], dtype = "uint8")
redUpper = np.array([120,120,255], dtype = "uint8")
RedCenter = (0,0) #(x,y)
BlueCenter = (0,0) #(x,y)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if len(cnts) > 0 and len(redcnts) > 0:
            ...
            redcnt = sorted(redcnts, key=cv2.contourArea, reverse=True)[
                0]  # sort in reverse order, so 0th is the largest
            ...
            if (isValid(redRect, blueRect)):
                ...
                cv2.putText(frame, ('Center %d,%d' % Center), (10, 15), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 255, 255),
                            2, cv2.LINE_AA)
                cv2.putText(frame, ('Orientation %d' % Angle), (10, 40), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 255, 255),
                            2,
                            cv2.LINE_AA)
